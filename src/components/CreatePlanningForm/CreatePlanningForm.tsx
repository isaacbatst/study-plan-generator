...
import {
  CreatePlanningFormSchema,
  CreatePlanningFormSchemaType,
} from "./CreatePlanningFormSchema";
import CreatePlanningFormSubjectSelect, {
  SubjectOption,
} from "./CreatePlanningFormSubjectSelect";
import { v4 } from "uuid";
...
export default function CreatePlanningForm({
  ...
  const [isDifferentHoursPerDay, setIsDifferentHoursPerDay] =
  ...
  const endDate = useMemo(() => {
    try {
      const parsing = CreatePlanningFormSchema.safeParse({
        ...
        startDate,
        subjects: selectedSubjects,
        availabilityPerWeekday: availabilityPerWeekday.map((value) => ({
          value,
        })),
      });
      if (!parsing.success) {
  ...
  async function onSubmit(data: CreatePlanningFormSchemaType) {
    try {
      ...
      toast("Copiado para a área de transferência", {
        description:
          "Agora você pode colar seu plano de estudos em qualquer lugar!",
  ...
  return (
    <div
      className={cn(
        "text-center xl:text-left flex justify-center items-center flex-1 px-3 py-10 max-w-full",
        {
    ...
    >
      <Form {...form}>
        <form
          ...
          className="space-y-4 flex flex-col flex-1 max-w-full"
        >
          {form.formState.errors.root?.error && (
            <p>{form.formState.errors.root.error.message}</p>
          )}
          <FormField
            control={form.control}
            name="subjects"
            render={({ field }) => (
              <CreatePlanningFormSubjectSelect
                field={field}
          ...
          <div className="flex flex-col gap-4">
            <FormField
              control={form.control}
              name="startDate"
              render={({ field }) => <CreatePlanningFormPeriod field={field} />}
            />
            {endDate && (
              <FormDescription>
                ...
                >
                  <CollapsibleContent>
                    <div className="flex flex-row flex-wrap space-y-2 space-x-2 justify-center">
                      {availabilityPerWeekdayFields.map((field, index) => (
                        <FormField
                          control={form.control}
                          key={field.id}
                          name={`availabilityPerWeekday.${index}.value`}
                          render={({ field }) => (
                            <CreatePlanningFormAvailabilityPerWeekday
                              field={field}
                              index={index}
                            />
                          )}
                  ...
                  <CollapsibleTrigger asChild>
                    <Button variant="outline" size="sm">
                      {isDifferentHoursPerDay
                        ? "Quero horas iguais todo dia"
                        : "Quero variar o número de horas por dia"}
                    </Button>
                  </CollapsibleTrigger>
                </Collapsible>
              </div>
