...
const Plannings = ({ subjects: defaultSubjects }: Props) => {
  ...
  const { plannings, isLoading, addStudyPlan, removeStudyPlan } =
    usePlanningsContext();
  const [showInitialForm, setShowInitialForm] = useState<boolean>(
    plannings.length === 0,
  );
  ...
  return (
    <div className="bg-gray-50 flex-1 flex flex-col gap-2 py-5 items-center justify-center">
      {isLoading && (
      ...
      {!isLoading && !showInitialForm && (
        <CreatePlanningFormDialog
          savePlanning={addStudyPlan}
      ...
      <div className="flex flex-col items-center gap-5 sm:py-5 flex-1 sm:flex-grow-0">
        ...
        {plannings.map((studyPlan) => (
          <PlanningView
            key={studyPlan.id}
            removeStudyPlan={removeStudyPlan}
            studyPlan={studyPlan}
          />
        ))}
      </div>
