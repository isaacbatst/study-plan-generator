...
export const PlanningsContextProvider = ({ children }: PropsWithChildren) => {
  const [studyPlans, setStudyPlans, isLoading] = useStoredState<PlanningJSON[]>(
    "study-plans",
    [],
  );
  ...
  const removeStudyPlan = useCallback(
    (id: string) => {
      ...
      toggleStudyObjectDone: (
        ...
        const newPlans = studyPlans.map((plan) => {
          ...
          const newStudyDays = plan.studyDays.map((studyDay) => {
            ...
            const newStudyObjects = studyDay.plannedStudyObjects.map(
              (planned) => {
  ...
  return (
    <PlanningsContext.Provider value={value}>
      {children}
...
export const usePlanningsContext = () => {
  ...
  if (!context) {
    throw new Error(
      "usePlanningsContext must be used within a PlanningsContextProvider",
    );
  }
  return context;
};
