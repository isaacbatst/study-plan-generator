...
export class PlanningStudyObject {
  ...
  get hoursLeft(): number {
    return (
      this.studyObject.getHours() -
      this.allocations.reduce((acc, allocation) => acc + allocation.hours, 0)
    );
  }

  static create(
    props: PlanningStudyObjectProps,
  ): Either<string, PlanningStudyObject> {
    return Either.right(new PlanningStudyObject(props.studyObject));
  }

  addAllocation(
    date: Date,
    hours: number = this.hoursLeft,
  ): Either<string, void> {
